...
class LoadConfig:
	...
	self.MaxPlSize = self.config['burning.maxplsize']
        ...
        self.fullscreen = int(self.read('gui.fullscreen', 1))
	...
	if self.joyType == 'asia': # for GreenAsia joy
        ...
        ### HERE WE START IMPLEMENTING READING CONFIG FROM DATABASE ###
        ### FIXME : should be adapted to the above code someday     ###

        self.get_configDB()

    def get_configDB(self):
        try:
            db = MySQLdb.connect(host=self.DBhost, user=self.DBuser, passwd=self.DBpass, db=self.DB)
            ...
            records =[]
            if len(result) == 0: print "No config found on the database!"
            for row in result:
                self.config[row[0]] = row[1]
        except MySQLdb.Error, e:
            if e[0] == 1049:
                #print "Database '%s' (read from /etc/burnstation/burnstation2.conf) does not exist! Please, read the INSTALL to learn how to install it." % self.DB
                print "Database '%s' (read from /etc/burnstation/burnstation2.conf) does not exist!" % self.DB
